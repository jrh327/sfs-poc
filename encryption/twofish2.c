...
typedef int cdecl CipherProc(cipherInstance* cipher, keyInstance* key, uint8_t* input, int inputLen, uint8_t* outBuffer);
...
extern uint32_t cdecl TwofishAsmCodeSize(void);
#endif
...
*/
#define Fe32_(x, R) (_sBox_[0][2 * _b(x, R)] ^ \
        _sBox_[0][2 * _b(x, R + 1) + 1] ^      \
        _sBox_[2][2 * _b(x, R + 2)] ^          \
...
#define VALIDATE_PARAMS 0 /* disable for full speed */
...
#define Xor32(dst, src, i) {                              \
        ((uint32_t*)dst)[i] = ((uint32_t*)src)[i] ^ tmpX; \
    }
...
void Xor256(void* dst, const void* src, uint8_t b) {
    register uint32_t x = b * 0x01010101u; /* replicate byte to all four bytes */
...
int reKey(keyInstance* key) {
    ...
    int subkeyCnt;
    uint32_t A = 0, B = 0, q;
    uint32_t sKey[MAX_KEY_BITS / 64], k32e[MAX_KEY_BITS / 64], k32o[MAX_KEY_BITS / 64];
...
int blockEncrypt(cipherInstance* cipher, keyInstance* key, const uint8_t* input,
    ...
    uint32_t sk[TOTAL_SUBKEYS];
    uint32_t IV[BLOCK_SIZE / 32];
...
int blockDecrypt(cipherInstance* cipher, keyInstance* key, const uint8_t* input,
    ...
    if (cipher->mode == MODE_CFB1) { /* use blockEncrypt here to handle CFB, one block at a time */
        ...
        for (n = 0; n < inputLen; n++) {
            ...
            outBuffer[n / 8] = (outBuffer[n / 8] & ~bit0) | (ctBit ^ ((((uint8_t*)x)[0] & 0x80) >> (n & 7)));
            carry = ctBit >> (7 - (n & 7));
            for (i = BLOCK_SIZE / 8 - 1; i >= 0; i--) {
                bit = cipher->IV[i] >> 7; /* save next "carry" from shift */
                cipher->IV[i] = (cipher->IV[i] << 1) ^ carry;
