...
uint32_t ReadTimeStampCounter(void);
...
typedef struct {
    ...
    uint8_t pt[BLOCK_SIZE / 8]; /* plaintext */
    uint8_t ct[BLOCK_SIZE / 8]; /* ciphertext */
...
uint32_t randBits[64] = { 1 }; /* use Knuth's additive generator */
int randPtr;
testData* debugTD = NULL; /* for use with debugIO */
int CLKS_uint8_t = 0; /* use clks/byte? (vs. clks/block) */
...
static char hexString[] = "0123456789ABCDEFFEDCBA987654321000112233445566778899AABBCCDDEEFF";

/*
+*****************************************************************************
*           Functions
-****************************************************************************/
uint32_t Here(uint32_t x) {
    unsigned int mask = ~0U;

    return (*(((uint32_t*)&x) - 1)) & mask;
}
extern uint32_t TwofishCodeSize(void);
...
void TimeOps(int iterCnt) {
    ...
    for (i = 0; i < TEST_CNT; i++) {
        if (needSet[i] & 1) {
            denom[i] = sizeof(text) / ((CLKS_uint8_t) ? 1 : (BLOCK_SIZE / 8));
            atomName[i] = (CLKS_uint8_t) ? "byte " : "block";
        }
        format[i] = (CLKS_uint8_t) ? "%10.1f/%s " : "%10.0f/%s ";
    }
...
#define TSCALE ((1.0E6 * CLK_MHZ) / CLOCKS_PER_SEC)
            ...
            { /* run a few times to get "best" time */
                ...
                case 2:
                    ...
                    CALL_N
                    {
                        t.ki.key32[0] += 0x87654321; /* change key bytes to force cache misses */
...
void AES_Sanity_Check(int testCnt) {
    ...
    uint8_t mode;
#if ALIGN32
    uint8_t alignDummy[3]; /* keep dword alignment on stack after uint8_t mode */
#endif
    ...
    if (testCnt) {
        for (mode = MODE_ECB; mode <= MODE_CFB1; mode++) {
            ...
            for (t.keySize = KEY_BITS_0; t.keySize <= MAX_KEY_BITS; t.keySize += STEP_KEY_BITS) {
                ...
                for (testNum = 0; testNum < lim; testNum++) { /* run a bunch of encode/decode tests */
                    ...
                    for (j = 0; j < n; j++) { /* set random plaintext */
                        pt[j] = (testNum) ? (uint8_t)Rand() : 0;
                    }
                    if (mode == MODE_CBC) { /* check that CBC works as advertised */
                        cipherInit(&t.ci, mode, ivString);
                        t.ci.mode = MODE_ECB;
                        for (q = 0; q < BLOCK_SIZE / 8; q++) { /* copy over the iv */
                            t.pt[q] = (uint8_t)(t.ci.iv32[q / 4] >> (8 * (q & 3))); /* auto-Bswap! */
                        }
...
FILE* AES_PutFileHeader(const char* fileName, const char* testName) {
    ...
    if (AES_FileIO(f, s, 1)) { /* header mismatch */
        if (!verify) {
...
void AES_printf(testData* t, const char* fmt) {
    ...
    for (s[1] = 0; *fmt; fmt++) {
        ...
        case 'S':
            ...
            break;
        case 'P':
            AES_PutBytes(t->f, "PT", t->pt, BLOCK_SIZE / 8, 0);
            break;
        case 'C':
            AES_PutBytes(t->f, "CT", t->ct, BLOCK_SIZE / 8, 0);
            break;
        case 'v':
            AES_PutBytes(t->f, "IV", t->ci.IV, BLOCK_SIZE / 8, 0);
            break;
        case 'V':
            AES_PutBytes(t->f, "IV", t->ci.iv32, BLOCK_SIZE / 8, 1);
            break;
        case 'K':
            AES_PutBytes(t->f, "KEY", t->ki.key32, t->keySize / 8, 1);
            break;
        default:
            s[0] = *fmt;
            s[1] = 0;
...
void AES_Test_Intermediate(const char* fname) {
    ...
    if ((useAsm) || (!debugCompile)) {
        if (!quietVerify) {
            printf("WARNING: Skipping %s test\n", fname);
        }
        return;
    }
        ...
        for (t.I = 0; t.I < mctOuter; t.I++) {
            ...
            for (j = 0; j < mctInner; j++) {
                ...
                if (j == mctInner - 1) { /* xor the key for next outer loop */
                    for (i = 0; i < t.keySize / 32; i++) {
                        t.ki.key32[i] ^= Bswap((i >= q) ? ((uint32_t*)t.ct)[i - q] : ((uint32_t*)t.pt)[BLOCK_SIZE / 32 - q + i]);
                    }
                }
                BlockCopy(t.pt, t.ct);
            }
    ...
    for (t.keySize = KEY_BITS_0, q = 0; t.keySize <= MAX_KEY_BITS; t.keySize += STEP_KEY_BITS, q += 2) {
        ...
        for (t.I = 0; t.I < mctOuter; t.I++) {
            ...
            for (j = 0; j < mctInner; j++) {
                ...
                if (j == mctInner - 1) { /* xor the key for next outer loop */
                    for (i = 0; i < t.keySize / 32; i++) {
                        t.ki.key32[i] ^= Bswap((i >= q) ? ((uint32_t*)t.pt)[i - q] : ((uint32_t*)t.ct)[BLOCK_SIZE / 32 - q + i]);
                    }
                }
                BlockCopy(t.ct, t.pt);
            }
...
#define CV t.ci.IV /* use t.ci.IV as CV */
    ...
    for (t.keySize = KEY_BITS_0, q = 0; t.keySize <= MAX_KEY_BITS; t.keySize += STEP_KEY_BITS, q += 2) {
        ...
        for (t.I = 0; t.I < mctOuter; t.I++) {
            ...
            for (i = 0; i < t.keySize / 32; i++) {
                t.ki.key32[i] ^= Bswap((i >= q) ? ((uint32_t*)t.ct)[i - q] : ((uint32_t*)ctPrev)[BLOCK_SIZE / 32 - q + i]);
            }
...
#define CV t.ci.IV /* use t.ci.IV as CV */
    ...
    for (t.keySize = KEY_BITS_0, q = 0; t.keySize <= MAX_KEY_BITS; t.keySize += STEP_KEY_BITS, q += 2) {
        ...
        for (t.I = 0; t.I < mctOuter; t.I++) {
            ...
            for (i = 0; i < t.keySize / 32; i++) {
                t.ki.key32[i] ^= Bswap((i >= q) ? ((uint32_t*)t.pt)[i - q] : ((uint32_t*)ptPrev)[BLOCK_SIZE / 32 - q + i]);
            }
            if (reKey(&t.ki) != TRUE) {
                FatalError("reKey return during %s test", fname);
            }
...
int ParseArgFile(const char* fName, char* argList[], int maxArgCnt) {
    ...
    if (debug) {
        printf("Reading args from file %s: ", fName);
    }

    for (argCnt = 0; argCnt < maxArgCnt;) { /* read in arg file one line at a time */
...
void Debug6805(void) {
    ...
    for (i = 0; i < 4; i++) { /* make sure it all fits in 256 bytes */
        ...
        for (j = 0; j < 16; j++) {
            ((uint8_t*)t.ki.key32)[j] = t.pt[j] ^ t.ct[j];
        }
...
#define MAX_ARGS 40
    ...
    uint32_t randSeed = 0x12345678;
    char* argList[MAX_ARGS];
    const char* moduleName = moduleDescription;
...
#error MCT loop counts incorrect!
        ...
        if (argList[i][0] == '-') {
            ...
            case 'F':
                ...
                case 'L':
                    ...
                    break;
                case 'B':
                    CLKS_uint8_t = ~CLKS_uint8_t;
                    break;
                case 'M':
                    CLK_MHZ = atoi(argList[i] + 3);
                    break;
                }
                break;
            case '?':
            ...
            case 'S':
                if (argList[i][2]) {
                    ...
                    randSeed = (uint32_t)time(NULL); /* randomize */
                }
                break;
            case 'T':
                if (argList[i][2]) {
...
#ifdef USE_ASM
    if (useAsm & 7)
        moduleName = "Assembler ";
#endif

    printf("%s%lu.%s: %s%s [%u bytes,%s].\n", CompilerName, 8 * sizeof(int),
#ifdef USE_ASM
    ...
    if (doTableTest) {
        ...
        printf("WARNING: Skipping ecb_tbl.txt verification\n");
    }
